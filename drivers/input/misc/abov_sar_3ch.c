            ...
            case IDLE: /* Button is being in far state! */
                ...
                if ((i & pCurrentButton->mask) == pCurrentButton->mask) {
                    ...
                    if (board->cap_channel_ch0 == counter) {
                        ...
                        input_report_abs(input_topright, ABS_DISTANCE, 2);
                        input_sync(input_topright);
                    } else if (board->cap_channel_ch2 == counter) {
                        input_report_abs(input_topleft, ABS_DISTANCE, 2);
                        input_sync(input_topleft);
                    }					
            ...
            case S_BODY: /* Button is being in 0mm! */
                if ((i & pCurrentButton->mask) == pCurrentButton->mask) {
                    ...
                    if ((board->cap_channel_ch0 == counter) && (this->channel_status & 0x01)) {
                        ...
                        input_report_abs(input_topright, ABS_DISTANCE, 0);
                        input_sync(input_topright);
                    } else if ((board->cap_channel_ch2 == counter) && (this->channel_status & 0x04)) {
                        input_report_abs(input_topleft, ABS_DISTANCE, 0);
                        input_sync(input_topleft);
                    }
                    pCurrentButton->state = S_PROX;
                } else{
                    LOG_INFO("CH%d State=IDLE.\n",counter);
                    if ((board->cap_channel_ch0 == counter) && (this->channel_status & 0x01)) {
                        input_report_abs(input_bottom, ABS_DISTANCE, 5);
                        input_sync(input_bottom);
                    } else if ((board->cap_channel_ch1 == counter) && (this->channel_status & 0x02)) {
                        input_report_abs(input_topright, ABS_DISTANCE, 5);
                        input_sync(input_topright);
                    } else if ((board->cap_channel_ch2 == counter) && (this->channel_status & 0x04)) {
                        input_report_abs(input_topleft, ABS_DISTANCE, 5);
                        input_sync(input_topleft);
                    }
                    pCurrentButton->state = IDLE;
                }
...
static ssize_t reg_dump_show_ch2(struct class *class,
        struct class_attribute *attr,
        char *buf)
{
    u8 reg_value = 0;
...
static ssize_t reg_dump_store_ch2(struct class *class,
        struct class_attribute *attr,
        const char *buf, size_t count)
{
    return count;
...
static CLASS_ATTR(reg_ch2, 0660, reg_dump_show_ch2, reg_dump_store_ch2);
