...
struct core_mp_test_data {
	...
	u32 protocol_ver;
	u32 core_ver;
	char ini_date[128]; //+add by songbinbo.wt for ilitek Open short test add config file release date 20191202
	int no_bk_shift;
	bool retry;
	bool m_signal;
	bool m_dac;
...
static void mp_print_csv_header(char *csv, int *csv_len, int *csv_line, int file_size)
{
	...
	tmp_line++;
    //+add by songbinbo.wt for ilitek Open short test add config file release date 20191202
...
static void mp_test_free(void)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (tItems[i].catalog == TX_RX_DELTA) {
			...
			ipio_kfree((void **)&tItems[i].node_type); //+add by songbinbo.wt for ilitek Open short test add config file release date 20191202
			ipio_kfree((void **)&tItems[i].result); //+add by songbinbo.wt for ilitek Open short test add config file release date 20191202
...
int ilitek_tddi_mp_test_main(char *apk, bool lcm_on, char *single)
{
	...
	ret = ilitek_tddi_mp_ini_parser(idev->ili_name);//add by songbinbo.wt for ILITEK tp driver compatible truly & txd vendor  20191005
	if (ret < 0) {
