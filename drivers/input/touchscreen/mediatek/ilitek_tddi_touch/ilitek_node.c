...
int wt_tp_openshort(void)
{
	...
	if(ret < 0){
		ipio_info("MP test fail\n");
		ret = -1;
	}else{
		ipio_info("MP test pass\n");
		ret = 0;
	}
/*
	if(idev->mp_result == false){
		ipio_info("MP test fail\n");
		ret = -1;
	}else{
		ipio_info("MP test pass\n");
...
//+add by songbinbo.wt for ilitek tp gesture & touch sensitivity mode 20191109

static ssize_t ilitek_proc_double_tap_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	uint32_t len = 0;
	...
	if (*pPos != 0)
		return 0;
	memset(g_user_buf, 0, USER_STR_BUFF * sizeof(unsigned char));
	len = snprintf(g_user_buf, USER_STR_BUFF * sizeof(unsigned char), "double_tap : %s\n", idev->gesture ? "Enable" : "Disable");
...
static ssize_t ilitek_proc_double_tap_write(struct file *filp, const char *buff, size_t size, loff_t *pPos)
{
	int res = 0;
	char cmd[10] = { 0 };
	 int temp =0;
...
static ssize_t ilitek_proc_glove_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	...
	if (*pPos != 0)
		return 0;
	memset(g_user_buf, 0, USER_STR_BUFF * sizeof(unsigned char));
	len = snprintf(g_user_buf, USER_STR_BUFF * sizeof(unsigned char), "glove_mode : %s\n",  idev->glove ? "Enable" : "Disable");
...
static ssize_t ilitek_proc_glove_write(struct file *filp, const char *buff, size_t size, loff_t *pPos)
{
	int res = 0;
	char cmd[10] = { 0 };
	int temp =0;
...
//+add by songbinbo.wt for ilitek tp gesture & touch sensitivity mode 20191109
struct file_operations proc_double_tap_enable_fops = {
	.read = ilitek_proc_double_tap_read,
	.write = ilitek_proc_double_tap_write,
};

struct file_operations proc_glove_enable_fops = {
...
//+add by songbinbo.wt for ilitek tp gesture & touch sensitivity mode 20191109
	{"double_tap_enable", NULL, &proc_double_tap_enable_fops, false},
	{"glove", NULL, &proc_glove_enable_fops, false},
//-add by songbinbo.wt for ilitek tp gesture & touch sensitivity mode 20191109
};
