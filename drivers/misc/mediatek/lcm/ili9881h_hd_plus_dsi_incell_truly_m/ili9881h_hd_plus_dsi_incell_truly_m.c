...
#define LCM_PHYSICAL_WIDTH									(68108)
#define LCM_PHYSICAL_HEIGHT									(143230)
...
static void lcm_init(void)
{
	...
	//+add by songbinbo.wt for ILITEK tp reset sequence 20190604
	lcd_load_ili_fw();
	//-add by songbinbo.wt for ILITEK tp reset sequence 20190604
	pr_debug("[LCM after	lcd_load_ili_fw.\n]");
	push_table(NULL, init_setting_vdo, sizeof(init_setting_vdo) / sizeof(struct LCM_setting_table), 1);
	//pr_debug("[LCM] ili9881h--truly-m--sm5109----init success ----\n");
...
static void lcm_setbacklight_cmdq(void *handle, unsigned int level)
{
	...
	pr_debug("[truly_m_ili9881h]para_list[0]=%x,para_list[1]=%x\n",bl_level[1].para_list[0],bl_level[1].para_list[1]);
...
{
	pr_err("[truly_m_ili9881h] cabc get to %d\n", cabc_status);
	*status = cabc_status;
}
...
struct LCM_DRIVER ili9881h_hd_plus_dsi_vdo_truly_m_lcm_drv = {
	.name = "ili9881h_hd_plus_vdo_truly_m",
#endif
