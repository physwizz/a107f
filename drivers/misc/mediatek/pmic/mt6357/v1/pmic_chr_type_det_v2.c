...
static bool charger_secdet_num = true;
#define SECOND_DETECT_DELAY_TIME	15 * 1000
//-Bug 510049, liuyong3, wt, ADD, 20191121, Add charger secdet for Non-standard charger
...
void mtk_pmic_enable_chr_type_det(bool en)
{
	...
	if (en) {
		...
		//+Bug 510049, liuyong3, wt, ADD, 20191121, Add charger secdet for Non-standard charger
		if((NONSTANDARD_CHARGER == g_chr_type) && true == charger_secdet_num) {
			schedule_delayed_work(&charger_secdet_work, msecs_to_jiffies(SECOND_DETECT_DELAY_TIME));
			charger_secdet_num = false;
			pr_info("Non standard charger type, start second check\n");
		}
		//-Bug 510049, liuyong3, wt, ADD, 20191121, Add charger secdet for Non-standard charger
	} else {
