...
#include <linux/hardware_info.h> //bug 493726, add NFC info, dulinyu.wt, 20191018
...
int sec_nfc_i2c_probe(struct i2c_client *client)
{
    ...
    hardwareinfo_set_prop(HARDWARE_NFC, "S3NRN82"); //Bug 513686 , Modify NFC info, dulinyu.wt, 20191120
    pr_info("%s success\n", __func__);
    return 0;
                    ...
                    enum sec_nfc_mode mode)
{
    ...
    if (mode != SEC_NFC_MODE_OFF) {
		...
		pr_info("lulinliang set clk request open : %d \n", clk_sw_ctr_ret);
#ifdef CONFIG_SEC_NFC_DEDICATED_CLK
        val |= 0xC0000000;
		...
		pr_info("lulinliang set clk request close : %d \n", clk_sw_ctr_ret);
#ifdef CONFIG_SEC_NFC_DEDICATED_CLK
        val &= ~0xC0000000;
        writel(val, info->clkctrl);
#endif
...
/*Bug 497009 , set nfc shutdown period, add, dulinyu.wt, 20191031 begin*/
...
/*Bug 497009 , set nfc shutdown period, add, dulinyu.wt, 20191031 end*/

static struct i2c_driver sec_nfc_driver = {
    .probe = sec_nfc_probe,
    .id_table = sec_nfc_id_table,
    .remove = sec_nfc_remove,
    .shutdown = sec_nfc_shutdown, //Bug 497009 , set nfc shutdown period, add, dulinyu.wt, 20191031
    .driver = {
...
{
    if(strstr(saved_command_line,"board_id=S96716AA1") || strstr(saved_command_line,"board_id=S96716BA1") ) {
...
{
	if(!strstr(saved_command_line,"board_id=S96716AA1") && !strstr(saved_command_line,"board_id=S96716BA1")) {
        printk("nfc is support, exit is execute !\n");
