...
int afc_pre_check(struct charger_manager *pinfo)
{
	...
	for (i = 0; i < 1500; i++) {
		...
		if (!afc_device->to_check_chr_type ||
			...
			battery_get_uisoc() < pinfo->data.afc_start_battery_soc ||
			battery_get_uisoc() >= pinfo->data.afc_stop_battery_soc) {
			pr_err("%s: stop, SOC = (%d, %d, %d), to_check_chr_type = %d, chr_type = %d \n",
				__func__, battery_get_uisoc(), pinfo->data.afc_start_battery_soc,
				pinfo->data.afc_stop_battery_soc, afc_device->to_check_chr_type, mt_get_charger_type());
			return 1;
		}
