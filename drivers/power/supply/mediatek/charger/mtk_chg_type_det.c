...
{
	struct mt_charger *mtk_chg = power_supply_get_drvdata(psy);
    //+Bug 513778,liuyong3.wt,ADD,20191128,add type "HVDCP" fro afc/pd/pe charger
...
#endif
    //-Bug 513778,liuyong3.wt,ADD,20191128,add type "HVDCP" for afc/pd/pe charger
    ...
    //+Bug 513778,liuyong3.wt,ADD,20191128,add type "HVDCP" fro afc charger
#ifdef CONFIG_MT6370_PMU_CHARGER
			if (mtk_is_pep_series_connect(cm) ||mtk_pdc_check_charger(cm) || afc_get_is_connect(cm))
				val->strval = "USB_HVDCP";
			else
#endif
    //-Bug 513778,liuyong3.wt,ADD,20191128,add type "HVDCP" for afc charger
				val->strval = "USB_DCP";
		}
		else{
			val->strval = "UNKNOWN";
