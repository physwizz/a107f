...
early_param("androidboot.hw_id", sec_hw_param_get_hw_rev);
					    ...
					    struct kobj_attribute *attr, char *buf)
{
	...
	if (reset_reason == RR_K || reset_reason == RR_D || reset_reason == RR_P) {
		info_size = snprintf((char *)buf, 64, "\"%s\":\"%s\"", sec_debug_extra_info_backup->item[INFO_REASON].key,
				sec_debug_extra_info_backup->item[INFO_REASON].val);
	}
